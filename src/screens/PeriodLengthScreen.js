...
export default function PeriodLength({ navigation }) {
  ...
  const signInAnonymously = async () => {
    ...
    // Get values from AsyncStorage from previous screens
    let lastStartDate = (await AsyncStorage.getItem('lastStartDate')) || today;
    let periodLength = (await AsyncStorage.getItem('periodLength')) || 5;
    let cycleLength = (await AsyncStorage.getItem('CycleLength')) || 28;
    ...
    await AsyncStorage.setItem('nextPeriodStartDate', nextPeriodStartDate.toISOString());
    ...
    firebaseAuth
      ...
      .then(() => {
        navigation.navigate('Main', { type: 'anonymous' });
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  return (
    <SafeAreaView style={styles.wrapper}>
      ...
      <View style={{ bottom: 30, position: 'absolute' }}>
        <Animatable.View ref={AnimationRefSecondary} animation="fadeIn" delay={100}>
          <ButtonSecondary
            title="Fyll i senare"
        ...
        <Animatable.View ref={AnimationRefPrimary} animation="fadeIn" delay={100}>
          <ButtonPrimary
            title="Fortsätt"
            onPress={() => {
              animationPrimary();
